...
class LocalAttentionModule(nn.Module):
    def __init__(self, in_channels, kernel_size=3, reduction=4, use_residual=True):
        """
        增强版局部注意力模块
        Args:
            in_channels (int): 输入通道数
            kernel_size (int): 局部卷积核大小，默认3
            reduction (int): 通道压缩比例，默认4
            use_residual (bool): 是否使用残差连接（True: residual + residual*attention，False: residual*attention）
        """
        super(LocalAttentionModule, self).__init__()
        assert in_channels % reduction == 0, \
            f"in_channels ({in_channels}) must be divisible by reduction ({reduction})"

        self.reduction = reduction
        self.use_residual = use_residual

        # 通道压缩
        self.reduce = nn.Conv2d(in_channels, in_channels // reduction, kernel_size=1)

        # 局部特征提取（可替换为可变形卷积等）
        self.local_conv = nn.Conv2d(
            in_channels // reduction, in_channels // reduction,
            kernel_size=kernel_size, padding=kernel_size // 2, groups=1  # 若需轻量化可改为 groups=in_channels//reduction
        )
        ...
        self.attention_gen = nn.Sequential(
            nn.Conv2d(in_channels, 1, kernel_size=1),  # 降维至单通道注意力图
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # 生成空间注意力权重
        attention = self.attention_gen(x)  # 形状 (B, 1, H, W)

        # 残差连接
        if self.use_residual:
            return residual + residual * attention  # 保留原始特征 + 注意力调制
        else:
            return residual * attention  # 原始特征调制
...
class ConvTransBlock(nn.Module):
    def __init__(self, conv_dim, trans_dim, head_dim, window_size, drop_path, type='W', input_resolution=None):
        ...
        self.conv_block = nn.Sequential(
            ...
            LayerNorm(self.conv_dim, data_format="channels_first"),
            nn.GELU(),
            nn.Conv2d(self.conv_dim, self.conv_dim, 3, 1, 1, bias=False),
            LayerNorm(self.conv_dim, data_format="channels_first"),
            LocalAttentionModule(in_channels=self.conv_dim)
        )
...
class NewRetinexSCUNet3_Single_Stage(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=32, level=2, num_blocks=[1, 1, 1]):
        super(NewRetinexSCUNet3_Single_Stage, self).__init__()
...
class NewRetinexSCUNet3(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1, 1, 1]):
        super(NewRetinexSCUNet3, self).__init__()
        self.stage = stage

        modules_body = [
            NewRetinexSCUNet3_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=2,
